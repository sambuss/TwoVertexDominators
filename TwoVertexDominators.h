// *********************************************************
// TwoVertexDominators.h -- 
//     Version 1.0, January 13, 2023
//     Version 2.0. October 25, 2023. Major revision, bug fix, and interface change.
// 
//     Author: Sam Buss (sbuss@ucsd.edu) with Elena Dubrova, Vijay Ganesh,
//           Albert Oliveras and Maxim Tselenko.
...
//  
// Output:
//    Return code:  1 if there is at least one two vertex dominator.
//                  0 if 3 vertex connected and thus there are no 1- or 2-vertex dominators.
//                 -1 if not 2-vertex connected (Data in SingleVertDominator).
//                 -2 if error condition, invalid input (generates an assert)
// 
//    VertListA - list of vertices that can be the "right" member of
//                 two vertex dominator.
//    VertListB - list of vertices that can be the "left" member of
//                 two vertex dominator.
//    Two vertex dominators consist of a member of VertListA and a
...
//                              which can be paired with vertNum to make a dominator
...
//    Any values vertNum1 and vertNum2 will form a two vertex dominator iff
...
//          vertices form a two vertex dominator.  
...
//          All TVD pairs have one member from PathA and one member
//          from PathB.        
// *********************************************************

#pragma once
#ifndef TWO_VERTEX_DOMINATORS_H
...
public:
    ...
    int CalcDominators( int N, const int predecessors[], const int predIndex[], int maxNumGroups = INT_MAX);
    int CalcDominators(const std::vector<int> predecessors, const std::vector<int> predIndex, int maxNumGroups = INT_MAX);
...
public:
    ...
    public:
        ...
        int vertNum;        // The index of a vertex as potential member of 2-vertex dominator
        int minPairIdx;     // Index of the first vertex in the other path whigh forms a TVD with this vertex.
        int maxPairIdx;     // Index of the last vertex in the other path whigh forms a TVD with this vertex.
...
public:    // If not 2-connected (return code -1), this is the single vertex dominator closest to the sink
    int SingleVertDominator;

private:
    // VertListA and VertListB hold the two lists of members of PathA and PathB
    //   that are part of TVDs. They also old the minVert and maxVert info
    ...
    //   unique. Nor are they needed internally for computation by CalcDominators.
...
// Main routine for calculating the two-vertex dominators
//   This is just a wrapper to accept std::vector's as inputs.
inline int TwoVertexDominators::CalcDominators(
    const std::vector<int> predecessors, const std::vector<int> predIndex, int maxNumGroups)
{
    ...
    return CalcDominators((int)predIndex.size(), predecessors.data(), predIndex.data(), maxNumGroups);
}

#endif // TWO_VERTEX_DOMINATORS_H