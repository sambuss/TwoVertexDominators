...
//
// TwoVertexDominators.cpp
//   Version 1.0, January 13, 2024
//   Version 1.1. October 17, 2024. Minor revisions & Phase C performance fix.
//   Version 2.0. October 30, 2024. Major revision, bug fix, and interface change.
//   Version 2.1. January 8, 2024. Bug fix.
// .
...
// 
// Algorithm is from:
//   * Maxim Teslenko and Elena Dubrova, "A Linear-Time Algorithm for
//     Finding All Double-Vertex Dominators of a Given Vertex", 
//     arxiv:1503.04994v1, 2015
//   * Sam Buss, Elena Dubrova, Vijay Ganesh, Albert Oliveras and Maxim Teslenko,
//     "A Linear Time Algorithm for Two-Vertex Dominators", technical report,
//     2024/2025.
// 
//  ************************************************
//  * Main routines for TwoVertexDominators        *
//  *                                              *
//  * See TwoVertexDominator.h for usage.          *
//  ************************************************
...
int TwoVertexDominators::CalcDominators(int N, const int predecessors[], const int predIndex[], int maxNumGroups)
{
    ...
    while (true) {
        ...
        if (backtrackVert == maxReachedOnPath) {
            SingleVertDominator = maxReachedOnPath;    // First dominator vertex.
    ...
    //     is not needed anymore by CalcDominators.
    PathA.clear();
    ...
    if (directReach1.back().maxDirectReachOnPath == N - 1) {
        return 0;           // Three connected. Hence no two vertex dominators!
    }

    // ++++++++++++++++++++++++++++++++++++++++++++++
    // Phase C-2.  
    // Build the lists of vertices for paths 1 and 2
    //   that could be in a two-vertex dominator,
    ...
    for (int leftFlag = 0; leftFlag <= 1; leftFlag++) {
        ...
        if (thisPathPivots.size() == 0) {
            return 0;                   // No potential TVD members. The graph is three connected.
        }
        thisPathPivots.emplace_back(N - 1, N - 1);
    }

    // ****************************************************
    // Phases D-1 & D-2 - Complete the analysis of two-vertex dominators.
    //   Fill in the information in VertListA and VertListB with
    //   the complete specification of allowable two vertex dominators
    ...
    //   such that (x,y) forms a two vertex dominator (TVD) pair.
    //   These upper and lower bounds may not actually be present as vertices 
    //   in the other path.
    //   At the same, remove all vertices that do not belong to a valid TVD pair.
    // 
    ...
    for (int leftFlag = 0; leftFlag <= 1; leftFlag++) {
        ...
        for ( ; curPivot->PivotVert != N - 1; curPivot++) {
            ...
            assert(upperBdPair < N - 1 || lowerBdPair <= butLastOtherPathPivotVert);
            if (lowerBdPair <= upperBdPair) {
    ...
    if (VertListA.empty()) {
        return 0;    // No two vertex dominators - Source and sink are three connected
    }

    // ****************************************************
    // Phase D-2. Replace the upper and lower bounds from D-1 on
    //     vertices that form TVD pairs with indices into the other vertList.
    //     The result is that vertex VertListA[i] and VertListB[j] form
    //     a two vertex dominator (TVD) pair iff
