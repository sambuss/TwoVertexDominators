...
//     by the routine CalcDominators(...) in TwoVertexDominators.cpp,
...
// 
//  It returns the two-vertex dominator consisting of nodes (a,b)
//     such that the sum/product/other combination of the activity levels 
//     of vertex a and vertex b is maximized.
// *********************************************************

#ifndef MOST_ACTIVE_TVD_H
#define MOST_ACTIVE_TVD_H
...
// 
//  must be defined before linkage. See examples in TVDtester.cpp
//  -----------------------
...
//
// FindMaxActivityTVD & FindMaxActivityTVD_idx
// 
//   - Input is the VertListA and VertListB from a TwoVertexDominators object;
...
//
// These FindMaxActivityTVD... routines should be called 
//     only if the TwoVertexDominators function found TVD's.

template<typename ACTIVITY>
ACTIVITY FindMaxActivityTVD_idx(
    ...
    int& return_a, int& return_b)
{
    int return_a_idx;
    int return_b_idx;
    ACTIVITY mostActivity = FindMaxActivityTVD_idx<ACTIVITY>(vertListA, vertListB, return_a_idx, return_b_idx);
